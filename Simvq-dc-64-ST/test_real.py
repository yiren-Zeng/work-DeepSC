# 测试 SimVQ-SwinTransformer 模型在不同 SNR 的 MS-SSIM 和 PSNR 值
...
def test_real():
    ...
    print("=" * 40)
    print(f"开始 SimVQ-SwinTransformer 支路真实环境测试 (Real Transmission Chain)")
    ...
    deepsc_model = DeepSC(
        ...
        stage_num_heads=cfg.STAGE_NUM_HEADS,
    ...
    checkpoint_path = os.path.join("/workspace/yi/work/Simvq-dc-64-ST/checkpoints/best_vq_deepsc.pth")
    ...
    # 初始化 LPIPS 模型（使用 VGG 网络，更贴近传统感知损失）
    lpips_fn = lpips.LPIPS(net='vgg').to(device)

    for snr in TEST_SNRS:
        print(f"\n正在测试 SNR = {snr} dB ...")

        mean_ms_ssim, mean_psnr, mean_lpips = evaluate_metrics_with_channel(
            deepsc_model, test_dataloader, cfg.NUM_EMBEDDINGS_LIST, snr, ldpc_code, device, lpips_fn)

        results[snr] = {'ms_ssim': mean_ms_ssim, 'psnr': mean_psnr, 'lpips': mean_lpips}
        print(f"SNR {snr} dB | SimVQ Avg MS-SSIM: {mean_ms_ssim:.4f} | Avg PSNR: {mean_psnr:.4f} dB | Avg LPIPS: {mean_lpips:.4f}")

    print("\n" + "=" * 40)
    print("=== SimVQ-SwinTransformer 最终测试结果 ===")
    print(f"Codebook K List: {cfg.NUM_EMBEDDINGS_LIST}")
    print("=" * 40)
    print(f"{'SNR (dB)':<10} | {'MS-SSIM':<10} | {'PSNR (dB)':<10} | {'LPIPS':<10}")
    print("-" * 11 + "|" + "-" * 12 + "|" + "-" * 11 + "|" + "-" * 11)
    for snr in TEST_SNRS:
        final_ssim = results[snr]['ms_ssim']
        final_psnr = results[snr]['psnr']
        final_lpips = results[snr]['lpips']
        print(f"{snr:<10} | {final_ssim:<10.4f} | {final_psnr:<10.4f} | {final_lpips:<10.4f}")


@torch.no_grad()
def evaluate_metrics_with_channel(model, loader, num_embeddings_list, target_snr, ldpc_code, device, lpips_fn):
    """包含完整物理层链路的 MS-SSIM、PSNR 和 LPIPS 联合评估 (SimVQ/SRC版本)"""
    ...
    for real_image in loader:
        ...
        # === 计算 LPIPS ===
        # LPIPS 要求输入在 [-1, 1] 范围，将 [0,1] 映射回 [-1,1]
        lpips_input1 = img1 * 2 - 1
        lpips_input2 = img2 * 2 - 1
        lpips_val = lpips_fn(lpips_input1, lpips_input2).item()
        lpips_scores.append(lpips_val)

    # 返回一个元组，同时包含三个指标的测试集平均分
    return np.mean(ms_ssim_scores), np.mean(psnr_scores), np.mean(lpips_scores)
