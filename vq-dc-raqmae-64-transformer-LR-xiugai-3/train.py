...
def main():
    ...
    for epoch in range(start_epoch, cfg.NUM_EPOCHS):
        ...
        for i, real_images in enumerate(train_dataloader):
            ...
            recon_loss, latent_loss, vq_loss, raq_loss = deepsc_loss_fn(
                real_images,
            ...
            total_raq_losses += raq_loss.item()
        ...
        avg_vq_raq = total_vq_raq_losses / steps_per_epoch
        avg_vq = total_vq_losses / steps_per_epoch
        avg_raq = total_raq_losses / steps_per_epoch


        writer.add_scalar("Loss/Train/MAE", avg_mae, epoch)
        writer.add_scalar("Loss/Train/VQ", avg_vq, epoch)
        writer.add_scalar("Loss/Train/RAQ", avg_raq, epoch)
        writer.add_scalar("Loss/Train/Total", avg_mae + avg_vq_raq, epoch) # 按理来说avg_vq_raq = avg_vq + avg_raq
        ...
        with torch.no_grad():
            for real_images in val_dataloader:
                ...
                recon_loss, _ , _ , _ = deepsc_loss_fn(
                    real_images,
                    out["reconstructed_images_src"],
                    out["reconstructed_images_raq"],
                    out["vq_losses_src"],
        ...
        checkpoint = {
            ...
            'rng_state': torch.get_rng_state(),  # 保存随机状态
