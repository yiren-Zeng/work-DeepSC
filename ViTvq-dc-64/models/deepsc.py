...
class DeepSC(nn.Module):
    """
    单层 U-Net + 纯 SRC 支路 + QBridge-ViT 量化器
    将传统 VQ 替换为带 Transformer 的 FVQ 量化，提升码本利用率
    """
    def __init__(self,
                 in_channels,
                 out_channels,
    ...
    def print_codebook_utilization(results, num_embeddings_list=None):
        ...
        print("\n" + "=" * 80)
        print("  码本利用率统计报告 (Codebook Utilization Report - QBridge-ViT)")
        print("=" * 80)
        ...
        for i in range(num_layers):
            ...
            s = results['src'][i]
            print(f"  [VQ-ViT] 活跃率: {s['active_ratio']:.2%}  |  "
        ...
        print(f"  [VQ-ViT] 平均活跃率: {src_avg:.2%}")
        print("=" * 80 + "\n")
