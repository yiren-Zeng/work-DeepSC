...
def main():
    ...
    print(f"  - QBridge-ViT 类型: {cfg.QB_TYPE}")
    print(f"  - 冻结 Embedding: {cfg.EMB_NOGRAD}")
    print("=" * 40)
    ...
    # 打印模型参数量
    total_params = sum(p.numel() for p in deepsc_model.parameters())
    trainable_params = sum(p.numel() for p in deepsc_model.parameters() if p.requires_grad)
    print(f"[Model] Total parameters: {total_params:,}")
    print(f"[Model] Trainable parameters: {trainable_params:,}")
    for i, vq in enumerate(deepsc_model.vector_quantizers):
        emb_params = sum(p.numel() for p in vq.embedding.parameters())
        qb_params = sum(p.numel() for p in vq.qbridge.parameters())
        emb_grad = sum(p.numel() for p in vq.embedding.parameters() if p.requires_grad)
        qb_grad = sum(p.numel() for p in vq.qbridge.parameters() if p.requires_grad)
        print(f"[VQ Layer {i}] Embedding: {emb_params:,} (grad: {emb_grad:,}), QBridge: {qb_params:,} (grad: {qb_grad:,})")
    ...
    if cfg.RESUME and os.path.exists(cfg.RESUME_PATH):
        ...
        if torch.cuda.is_available() and checkpoint['cuda_rng_state'] is not None:
            try:
                ...
                print(f"[Warning] Failed to restore CUDA RNG states (GPU count mismatch): {e}")
