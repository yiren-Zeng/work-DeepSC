...
class DeepSC(nn.Module):
    """
    单层 U-Net + 纯 SRC 支路 + QBridge-ViT 无压缩版量化器
    将 VQBridge 的隐式压缩-恢复结构去掉，改为直接 token 序列上的自注意力
    """
    def __init__(self,
                 in_channels,
                 out_channels,
    ...
    def compute_codebook_utilization(self, dataloader, max_batches=None, device=None):
        ...
        for i in range(num_layers):
            ...
            # 计算变换后码本的码字间最小L2距离与坍缩统计
            src_codebook = self.vector_quantizers[i]._get_transformed_codebook()
    ...
    def print_codebook_utilization(results, num_embeddings_list=None):
        ...
        print("\n" + "=" * 80)
        print("  码本利用率统计报告 (Codebook Utilization Report - QBridge-ViT NoCompress)")
        print("=" * 80)
        ...
        for i in range(num_layers):
            ...
            s = results['src'][i]
            print(f"  [VQ-ViT-NoCompress] 活跃率: {s['active_ratio']:.2%}  |  "
        ...
        print(f"  [VQ-ViT-NoCompress] 平均活跃率: {src_avg:.2%}")
        print("=" * 80 + "\n")
