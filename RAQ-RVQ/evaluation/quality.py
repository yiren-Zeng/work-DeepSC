...
def _rvq_layout(out):
    ...
    if not isinstance(indices, (list, tuple)) or not isinstance(k_lists, (list, tuple)):
        raise ValueError(
            "Independent RAQ-RVQ evaluation requires nested 'indices' and 'rvq_k_lists'."
        )
    ...
    return [[int(k) for k in ks] for ks in k_lists]
...
def _accumulate_rvq_quantization(records, out, real_image):
    ...
    if not isinstance(diagnostics, (list, tuple)):
        raise ValueError(
            "Independent RAQ-RVQ evaluation requires model-side 'rvq_diagnostics'."
        )
...
def _build_diagnostics(
    ...
    if modulation is not None:
        ...
        diagnostics["modulation_bits_per_symbol"] = _MODULATION_BITS[
            modulation
        ]
        ...
        if stream_packing == "combined":
            ...
            for scale_idx, (stage_indices, stage_ks) in enumerate(
                ...
                for stage_idx, (indices, num_embeddings) in enumerate(
                    ...
                    flat_bits = np.asarray(
                        flat_bits, dtype=np.uint8
                    ).reshape(-1)
                    segments.append(
                        (
                            scale_idx,
            ...
            for (
                ...
                records.setdefault(
                    key,
                    _new_stage_record(
                        scale_idx, stage_idx, num_embeddings
                    ),
                )
            ...
            if offset != len(decoded_bits):
                raise RuntimeError(
                    "Combined-stream decoded payload was not consumed "
                    f"exactly: {offset} != {len(decoded_bits)}."
                )
        else:
