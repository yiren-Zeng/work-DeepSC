# 测试 VQ 模型在不同 SNR 的 MS-SSIM 和 PSNR 值
...
def test_real():
    ...
    print("=" * 40)
    print(f"开始 VQ 支路真实环境测试 (Real Transmission Chain)")
    ...
    checkpoint_path = os.path.join("/workspace/yi/work/Vq-dc-64-withoutber/checkpoints/best_vq_deepsc.pth")
    ...
    for snr in TEST_SNRS:
        ...
        print(f"SNR {snr} dB | VQ Avg MS-SSIM: {mean_ms_ssim:.4f} | Avg PSNR: {mean_psnr:.4f} dB")
...
def evaluate_metrics_with_channel(model, loader, num_embeddings_list, target_snr, ldpc_code, device):
    """包含完整物理层链路的 MS-SSIM 和 PSNR 联合评估 (VQ/SRC版本)"""
