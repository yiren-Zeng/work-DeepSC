# MS-SSIM 和 PSNR 评价指标（单支路 VQ：indices_src + 固定码本大小列表）
...
def evaluate_metrics_with_channel(model, loader, num_embeddings_list, target_snr, ldpc_code, device):
    """完整物理层链路的 MS-SSIM 与 PSNR（基于 VQ 索引比特流）。"""
    ...
    for real_image in loader:
        ...
        flat_bits, original_spatial_dims, original_num_embeddings = indices_to_bits(
            out["indices_src"], num_embeddings_list
        )
        ...
        reconstructed_images = model.reconstruct_from_indices(recovered_indices_list, codebooks=None)
