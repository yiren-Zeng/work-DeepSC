...
from .raq_module.api import RAQ # 是不是有毒？加个.就可以导入get_quantized_features这个新加的方法了

class VQDeepSC(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
    ...
    def sample_trg(self, min_k, max_k):
    ...
    def forward_train_raq(self, x):
        ...
        for i, feat in enumerate(encoder_features):
            vq_loss, quantized, perplexity, encoding_idx = self.vector_quantizers[i](feat)

            vq_losses_src.append(vq_loss)
            quantized_src.append(quantized) # 这里的向量量化后的特征
            perplexities_src.append(perplexity)
        ...
        for i, feat in enumerate(encoder_features):
            # 假设 self.raq 暴露了下/上界
            K_tilde = self.sample_trg(self.raq_min_trg, self.raq_max_trg)
            W_trg = self.raqs[i].generate_codebook_seq2seq(K_tilde) # [K̃, D_i]
            # 用你自带的量化器计算 VQ 损失，与主干口径一致
            vq_loss_t, quantized_t, encoding_idx_t = self.vector_quantizers[i].forward_with_weight(feat, W_trg)
        ...
        return {
            ...
            "perplexities_src": perplexities_src,
    ...
    def forward_test_mask_layers(self, x, trg_list, active_indices=None):
        ...
        for i, feat in enumerate(encoder_features):
            ...
            if target_k is None:
                ...
                # 使用 RAQ 码本
                W_current = self.raqs[i].generate_codebook_seq2seq(target_k)

            # 量化
            _, quantized_t, _ = self.vector_quantizers[i].forward_with_weight(feat, W_current)
            quantized_raq.append(quantized_t)
    ...
    def reconstruct_from_indices_src(self, all_encoding_indices):
        """
        Reconstructs the image from the given encoding indices.
        This method simulates the decoding process after channel transmission.
        """
        quantized_features = []
        for i, encoding_indices in enumerate(all_encoding_indices): # all_encoding_indices是一个列表，每个元素都是一个张量，每一个张量表示该层的的编码索引

            quantized = self.vector_quantizers[i].get_quantized_features_src(encoding_indices) # 得到该层的量化特征
            quantized_features.append(quantized)

        reconstructed_image = self.semantic_decoder(quantized_features)
        return reconstructed_image

    def reconstruct_from_indices_raq(self, all_encoding_indices, codebooks):
        """
        使用指定的码本重建 RAQ 图像
        ...
        for i, encoding_indices in enumerate(all_encoding_indices):
            # 使用指定的码本而不是 RAQ 的固定码本
            quantized = self.raqs[i].get_quantized_features_raq(encoding_indices, codebooks[i])
            quantized_features.append(quantized)
