...
class DeepSC(nn.Module):
    ...
    def forward_train_raq(self, x, trg_list=None):
        ...
        vq_losses_raq = []
        indices_raq = []
        codebooks_trg_list = []  # <--- 【新增1】：创建一个空列表用来装 W_trg
        ...
        for i, feat in enumerate(encoder_features):
            ...
            codebooks_trg_list.append(W_trg)  # <--- 【新增2】：把本层生成的目标码本装进列表里
        ...
        return {
            ...
            "W_src_list": codebooks_src_list,  # <--- 【新增4】：把源码本列表传出去
            "W_trg_list": codebooks_trg_list  # <--- 【新增5】：把目标码本列表传出去
        }
