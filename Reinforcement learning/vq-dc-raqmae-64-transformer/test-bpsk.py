...
from communications.modulation import bpsk_modulate, bpsk_demodulate
    ...
    for snr_db in snr_range_db:
        ...
        with torch.no_grad(): # 禁用梯度计算以节省内存
            for i, real_image in enumerate(test_dataloader):
                ...
                print(f"RAQ-Best-Actual bit length: {len(flat_bits_raq_best)}")
                ...
                # 对于BPSK，LLR = 2 * y / sigma^2。如果信号功率为1，则sigma^2 = 1/snr_linear
                # 因此，LLR = 2 * y * snr_linear
                # 对于AWGN，received_signal = modulated_signal + noise。噪声功率 = N0/2 每维度。
                # Es/N0 = 1/sigma^2。对于BPSK，Es=1。因此sigma^2 = 1/snr_linear。
                # LLR = 2 * received_signal / sigma^2 = 2 * received_signal * snr_linear
                # 对于实值BPSK，LLR简单地是 2 * received_signal / noise_variance
                # noise_variance = 1 / snr_linear
                llr_src = 2 * received_signal_src.real * snr_linear  # 计算LLR（对数似然比），是一个一维的张量，加了个取实部的操作
                llr_raq = 2 * received_signal_raq.real * snr_linear  # 计算LLR（对数似然比），是一个一维的张量，加了个取实部的操作
                llr_raq_best = 2 * received_signal_raq_best.real * snr_linear
