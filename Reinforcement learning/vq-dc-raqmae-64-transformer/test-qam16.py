...
def test(use_ldpc=True):
    ...
    for snr_db in snr_range_db:
        ...
        with torch.no_grad(): # 禁用梯度计算以节省内存
            for i, real_image in enumerate(test_dataloader):
                ...
                # ========== 3. 调制(QAM16) ==========
                modulated_signal_src = qam16_modulate(bits_tensor_src)
                modulated_signal_raq = qam16_modulate(bits_tensor_raq)
                modulated_signal_raq_best = qam16_modulate(bits_tensor_raq_best)
                ...
                if use_ldpc:
                    # 使用精确的16-QAM LLR计算
                    llr_src = qam16_llr(received_signal_src, snr_db, device)
                    llr_raq = qam16_llr(received_signal_raq, snr_db, device)
                    llr_raq_best = qam16_llr(received_signal_raq_best, snr_db, device)
                else:
                    # 硬判决
                    demodulated_bits_src = qam16_demodulate(received_signal_src)
                    demodulated_bits_raq = qam16_demodulate(received_signal_raq)
                    demodulated_bits_raq_best = qam16_demodulate(received_signal_raq_best)

                print("SRC-解调比特长度: ", len(llr_src) if use_ldpc else len(demodulated_bits_src))
                print("RAQ-解调比特长度: ", len(llr_raq) if use_ldpc else len(demodulated_bits_raq))
                print("RAQ-Best-解调比特长度: ", len(llr_raq_best) if use_ldpc else len(demodulated_bits_raq_best))

                # ========== 6. LDPC解码或硬判决(就是没有LDPC编码的情况下才硬判决） ==========
                if use_ldpc:
                    # 使用LLR进行软解码
    ...
    # 创建结果目录
    results_dir = "./test_results_qam16"
...
if __name__ == "__main__":
    # 可以选择是否使用LDPC编码
    use_ldpc = True  # 设置为False则不使用LDPC编码
