"""Adaptive arithmetic-coded masks for independent RAQ-RVQ Top-K."""
...
def pack_topk_arithmetic_mask_segments(
    ...
    """Pack Top-K indices with one arithmetic-coded mask per scale."""
    ...
    for scale, (stages, mask, stage_ks) in enumerate(
        ...
        mask_bits, arithmetic = encode_adaptive_binary_mask(mask_flat)
        second_bits = _values_to_bits(active_second, second_width)
        names = {
            "first": f"first_s{scale}",
            "mask_arithmetic": f"mask_arithmetic_s{scale}",
        ...
        scales.append(
            {
                ...
                "segment_source_bits": {
                    ...
                    "mask_arithmetic": int(mask_bits.size),
    ...
    mask_bits = sum(
        int(scale["segment_source_bits"]["mask_arithmetic"])
        for scale in scales
    )
    second_bits = sum(
    ...
    metadata = {
        "schema": "independent_raq_rvq_topk_arithmetic_mask_combined_v1",
        ...
        "arithmetic_mask_bits": mask_bits,
    ...
    if int(metadata["source_bits"]) != offset:
        raise RuntimeError("arithmetic combined source accounting mismatch")
    return segments, metadata


def unpack_topk_arithmetic_mask_combined(
    decoded_bits: np.ndarray,
    metadata: Dict[str, object],
) -> Tuple[List[List[torch.Tensor]], List[torch.Tensor], Dict[str, object]]:
    """Decode arithmetic masks and map the compact second-stage payload."""

    if metadata.get("schema") != (
        "independent_raq_rvq_topk_arithmetic_mask_combined_v1"
    ):
        raise ValueError("unsupported arithmetic-mask transport schema")
    ...
    for scale_metadata in metadata["scales"]:
        ...
        mask = decode_adaptive_binary_mask(
            decoded[names["mask_arithmetic"]], token_count
        )
...
@torch.no_grad()
def prepare_topk_arithmetic_mask_packets(
    model,
    ...
    """Apply Top-K and prepare arithmetic-mask packets."""
    ...
    for image_index, sample in enumerate(samples):
        if not (
            ...
            raise ValueError("adaptive sample scale layouts do not align")
...
def summarize_arithmetic_mask_packets(
    packets: Sequence[PreparedAdaptivePacket],
    ldpc_code: Dict[str, object],
    modulation: str,
) -> Dict[str, object]:
    """Summarize arithmetic-mask source and combined physical lengths."""
    ...
    totals = {
        ...
        for name in ("arithmetic", "raw", "dense")
    }
    total_pixels = 0
    total_values = 0
    ...
    for packet in packets:
        ...
        references = {
            "arithmetic": int(packet.metadata["source_bits"]),
        ...
        for scale_metadata in packet.metadata["scales"]:
            ...
            arithmetic_bits = int(segment_bits["mask_arithmetic"])
            ...
            stats["arithmetic_mask_source_bits"] += arithmetic_bits
    ...
    for stats in per_scale:
        ...
        arithmetic_values = stats.pop("arithmetic_bits_per_image")
        ...
        arithmetic_mask = int(stats["arithmetic_mask_source_bits"])
        stats.update(
            {
                ...
                "arithmetic_mask_source_bits_mean_per_image": (
                    arithmetic_mask / len(packets)
                ),
                "arithmetic_mask_source_bits_min_per_image": min(
                    arithmetic_values
                ),
                "arithmetic_mask_source_bits_max_per_image": max(
                    arithmetic_values
                ),
                "mask_source_bits_saved_vs_raw": raw_mask - arithmetic_mask,
                "mask_source_saving_ratio": (
                    (raw_mask - arithmetic_mask) / raw_mask
                ),
            }
        )

    arithmetic = totals["arithmetic"]
    raw = totals["raw"]
    dense = totals["dense"]
    return {
        ...
        "arithmetic_mask_bits": sum(
            int(scale["arithmetic_mask_source_bits"]) for scale in per_scale
        ),
        "raw_mask_bits_reference": sum(
            int(scale["raw_mask_source_bits_reference"])
            for scale in per_scale
        ),
        "active_second_bits": sum(
            int(scale["active_second_source_bits"]) for scale in per_scale
        ),
        "arithmetic_source_bits": arithmetic["payload_bits"],
        "arithmetic_source_bpp": arithmetic["payload_bits"] / total_pixels,
        "arithmetic_coded_bits": arithmetic["coded_bits"],
        "arithmetic_coded_bpp": arithmetic["coded_bits"] / total_pixels,
        "arithmetic_channel_symbols": arithmetic["channel_symbols"],
        "arithmetic_transmission_ratio_per_rgb_value": (
            arithmetic["channel_symbols"] / total_values
        ),
        "arithmetic_ldpc_padding_bits": arithmetic["ldpc_padding_bits"],
        "raw_explicit_reference": raw,
        "dense_two_stage_reference": dense,
        "source_bits_saved_vs_raw_mask": (
            raw["payload_bits"] - arithmetic["payload_bits"]
        ),
        "coded_bits_saved_vs_raw_mask": (
            raw["coded_bits"] - arithmetic["coded_bits"]
        ),
        "per_scale": per_scale,
...
@torch.no_grad()
def evaluate_arithmetic_mask_packets_over_channel(
    model,
    packets: Sequence[PreparedAdaptivePacket],
    snr_db: float,
    ldpc_code: Dict[str, object],
    ...
    """Transmit and decode arithmetic-mask combined packets."""
    ...
    num_scales = len(packets[0].metadata["scales"])
    kind_bits = {"first": 0, "mask_arithmetic": 0, "second_active": 0}
    kind_errors = {"first": 0, "mask_arithmetic": 0, "second_active": 0}
    per_scale = [
        {
            "scale": scale,
            "tx_active_count": 0,
            ...
            "arithmetic_source_bits": 0,
            "arithmetic_source_bit_errors": 0,
    ...
    for image_index, packet in enumerate(packets):
        ...
        for segment_metadata in packet.metadata["segments"]:
            ...
            elif kind == "mask_arithmetic":
                per_scale[scale]["arithmetic_source_bits"] += bits
                per_scale[scale]["arithmetic_source_bit_errors"] += errors
            else:
                per_scale[scale]["second_source_bits"] += bits
                per_scale[scale]["second_source_bit_errors"] += errors

        recovered, rx_masks, decode_stats = (
            unpack_topk_arithmetic_mask_combined(decoded, packet.metadata)
        )
        image_scale_records = []
        for scale, (tx_mask, rx_mask, scale_decode) in enumerate(
            ...
            mask_record = segment_records[names["mask_arithmetic"]]
            ...
            image_scale_records.append(
                {
                    ...
                    "arithmetic_source_bits": mask_record["source_bits"],
                    "arithmetic_source_bit_errors": mask_record[
                        "source_bit_errors"
                    ],
    ...
    for stats in per_scale:
        ...
        mask_bits = int(stats["arithmetic_source_bits"])
        second_bits = int(stats["second_source_bits"])
        source_bits = first_bits + mask_bits + second_bits
        source_errors = (
            int(stats["first_source_bit_errors"])
            + int(stats["arithmetic_source_bit_errors"])
        ...
        stats.update(
            {
                ...
                "arithmetic_source_ber_after_ldpc": (
                    stats["arithmetic_source_bit_errors"] / mask_bits
                ),
