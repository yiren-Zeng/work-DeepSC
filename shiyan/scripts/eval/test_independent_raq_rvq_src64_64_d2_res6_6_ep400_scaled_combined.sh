#!/bin/bash
# Evaluate the ep400-scaled checkpoint at the three exact-1/48 profiles.
set -euo pipefail
...
run_profile() {
  local profile="$1"
  shift
  local snr modulation ldpc_k k_lists json_output
  case "$profile" in
    ldpc12_qpsk)
      snr=3
      modulation=qpsk
    ...
    *)
      ...
      echo "Use: all, ldpc12_qpsk, ldpc34_qpsk, or ldpc12_16qam" >&2
      return 2
      ;;
  esac

  json_output="experiments/eval/independent_raq_rvq_src64-64_res6-6_ep400_scaled_${profile}_rate1over48_combined.json"
  export SIMVQ_INDEPENDENT_RAQ_RVQ_K_LISTS="$k_lists"
  local args=(
  ...
  echo "Profile: $profile | SNR=$snr | LDPC=$ldpc_k | modulation=$modulation | K=$k_lists | ratio=1/48"
  python -u test_real.py "${args[@]}" "$@"
}

case "$PROFILE" in
  all)
    run_profile ldpc12_qpsk "$@"
    run_profile ldpc34_qpsk "$@"
    run_profile ldpc12_16qam "$@"
    ;;
  *)
    run_profile "$PROFILE" "$@"
    ;;
