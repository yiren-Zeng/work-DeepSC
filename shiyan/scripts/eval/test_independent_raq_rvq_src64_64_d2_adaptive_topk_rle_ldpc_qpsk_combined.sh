#!/bin/bash
# Independent RAQ-RVQ: per-image/per-scale Top-K stage-2 RLE masks,
# one combined LDPC 1/2 + QPSK + AWGN stream per image by default.
set -euo pipefail
...
RVQ_K_LISTS="${RVQ_K_LISTS:-32,64;8,2}"
...
OUTPUT_DIR="${OUTPUT_DIR:-experiments/eval/independent_raq_rvq_src64_64_d2_adaptive_topk_rle_combined}"
...
echo "Mask: fixed-width row-major RLE, no raw-mask fallback"
...
python -u test_independent_raq_rvq_adaptive_topk_rle.py \
