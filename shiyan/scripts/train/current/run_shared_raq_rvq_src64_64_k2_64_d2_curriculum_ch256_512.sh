#!/bin/bash
# Strict fusion: one RAQ-generated codebook per scale, shared by two residual depths.
set -euo pipefail
...
export SIMVQ_EXP_FAMILY="shiyan_shared_raq_rvq_src64-64_trg2-64_d2_curriculum_rate094_A_patch_ch256-512"
...
# Additive trained mode.  Legacy dynamic/zero-shot RAQ-RVQ paths stay disabled.
export SIMVQ_USE_RAQ="1"
export SIMVQ_USE_SHARED_RAQ_RVQ="1"
export SIMVQ_SHARED_RAQ_RVQ_DEPTH="2"
...
# Match the original RAQ joint-training and reconstruction-gradient contract.
...
# Training samples K independently per scale from the curriculum.  This pair is
# used only for deterministic validation with a 9216-bit fused target payload.
export SIMVQ_RAQ_TARGET_LIST="${SIMVQ_RAQ_TARGET_LIST:-16,4}"
...
RUN_ID="shared-raq-rvq-src64-64-k2-64-d2-ch256-512_gpu${GPU_ID}-$(date +%Y%m%d-%H%M%S)"
...
echo "Shared RAQ-RVQ: enabled=$SIMVQ_USE_SHARED_RAQ_RVQ depth=$SIMVQ_SHARED_RAQ_RVQ_DEPTH"
echo "Codebook policy: one generated W per scale; shared across depths; no reserved zero"
echo "Gradient mode: $SIMVQ_RAQ_RECON_GRAD_MODE (one final STE)"
echo "Train branch: $SIMVQ_TRAIN_BRANCH (source + fused target joint loss)"
echo "Validation K: $SIMVQ_RAQ_TARGET_LIST"
