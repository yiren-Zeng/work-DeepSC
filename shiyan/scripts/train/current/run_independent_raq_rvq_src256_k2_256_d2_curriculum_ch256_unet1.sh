#!/bin/bash
# Two trained RAQ codebooks: one scale x two independent residual stages.
set -euo pipefail
...
# Deterministic validation layout for one scale and two residual stages.
export SIMVQ_INDEPENDENT_RAQ_RVQ_K_LISTS="16,4"
...
# The flat value is retained as nominal per-scale metadata. The nested two-K
...
RUN_ID="independent-raq-rvq-src256-k2-256-d2-ch256-unet1_gpu${GPU_ID}-$(date +%Y%m%d-%H%M%S)"
...
echo "Training policy: two independent curriculum K samples per accumulation"
echo "Validation two K: $SIMVQ_INDEPENDENT_RAQ_RVQ_K_LISTS"
echo "Validation payload: 6144 bits/image = 0.093750 bpp"
echo "Validation transmission ratio (LDPC1/2+BPSK): 0.06250000"
