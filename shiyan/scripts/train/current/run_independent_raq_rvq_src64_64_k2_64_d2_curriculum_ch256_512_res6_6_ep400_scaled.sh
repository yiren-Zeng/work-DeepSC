#!/bin/bash
# Four trained RAQ codebooks with all 400-epoch schedules scaled together.
set -euo pipefail
...
export NUM_EPOCHS="${NUM_EPOCHS:-400}"
export SIMVQ_LEARNING_RATE_G="5e-5"
export SIMVQ_CODEBOOK_PROJ_LR="2e-4"
# Match the relative timing of the successful 200-epoch schedule:
# channel ramp 40%-60%, StepLR decay every 50% of the total epochs.
export SIMVQ_CHANNEL_PROB_START_EPOCH="${SIMVQ_CHANNEL_PROB_START_EPOCH:-$((NUM_EPOCHS * 40 / 100))}"
export SIMVQ_CHANNEL_PROB_END_EPOCH="${SIMVQ_CHANNEL_PROB_END_EPOCH:-$((NUM_EPOCHS * 60 / 100))}"
export SIMVQ_LR_STEP_SIZE="${SIMVQ_LR_STEP_SIZE:-$((NUM_EPOCHS * 50 / 100))}"
...
RUN_ID="independent-raq-rvq-src64-64-k2-64-d2-ch256-512-res6-6-ep400-scaled_gpu${GPU_ID}-$(date +%Y%m%d-%H%M%S)"
...
echo "Scaled schedule: channel=$SIMVQ_CHANNEL_PROB_START_EPOCH-$SIMVQ_CHANNEL_PROB_END_EPOCH LR-step=$SIMVQ_LR_STEP_SIZE"
