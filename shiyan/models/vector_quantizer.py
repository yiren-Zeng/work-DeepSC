...
class VectorQuantizer(nn.Module):
    ...
    def forward_raq(self, inputs: torch.Tensor, embed_weight: torch.Tensor):
        ...
        The source SimVQ path still owns the learned source codebook. RAQ passes
        a dynamic target codebook generated from that source codebook, so this
        method mirrors ``forward`` while using ``embed_weight`` for nearest
        neighbor lookup and feature reconstruction.
        """
        inputs_bhwc = inputs.permute(0, 2, 3, 1).contiguous()
        B, H, W, C = inputs_bhwc.shape
        flat = inputs_bhwc.view(-1, C)
    ...
    def get_quantized_features(
        ...
        codebook_weight: torch.Tensor = None,
    ) -> torch.Tensor:
        ...
        weight = self.transformed_weight() if codebook_weight is None else codebook_weight
...
class ChannelwiseVectorQuantizer(VectorQuantizer):
    ...
    def get_quantized_features(
        ...
        if codebook_weight is not None:
            raise ValueError("RAQ external codebooks are only implemented for patch-wise quantizers.")
...
class VanillaVectorQuantizer(nn.Module):
    ...
    def get_quantized_features(
        ...
        if codebook_weight is not None:
            raise ValueError("RAQ external codebooks are only implemented for SimVQ patch quantizers.")
        if encoding_indices.dim() == 2:
            encoding_indices = encoding_indices.unsqueeze(0)
        B, H, W = encoding_indices.shape
        flat_idx = encoding_indices.reshape(-1)
