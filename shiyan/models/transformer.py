import torch
import torch.nn as nn
import torch.nn.functional as F
import math


class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=5000):
        super().__init__()
        self.d_model = d_model
        self.register_buffer('pe', self._build_pe(max_len, d_model))

    @staticmethod
    def _build_pe(max_len, d_model, device=None):
        pe = torch.zeros(max_len, d_model, device=device)
        position = torch.arange(0, max_len, dtype=torch.float, device=device).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        div_term = div_term.to(device=device)
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        return pe.unsqueeze(1)

    def _ensure_length(self, seq_len, device):
        if self.pe.size(0) >= seq_len and self.pe.device == device:
            return
        new_len = max(seq_len, self.pe.size(0) * 2)
        self.pe = self._build_pe(new_len, self.d_model, device=device)

    def forward(self, x):
        self._ensure_length(x.size(0), x.device)
        return x + self.pe[:x.size(0), :].to(dtype=x.dtype)


class TransformerCodebookGen(nn.Module):
    def __init__(self,
                 src_dim: int, # VQ权重的原始维度
                 embed_layer_dec: nn.Embedding, # 为投影层输出的最大长度RAQ码本字典
                 d_model: int,  #  embedding_dim
                 nhead: int = 8,  # 设为 8，可以整除 128, 256, 512, 1024
                 num_encoder_layers: int = 3,
                 num_decoder_layers: int = 3,
                 dim_feedforward: int = 1024,
                 dropout: float = 0.1,
                 device: str = "cuda:2",
                 generator_type: str = "encoder_decoder",
                 max_len: int = 5000):
        super().__init__()

        self.device = torch.device(device)
        self.embed_layer_dec = embed_layer_dec
        self.generator_type = str(generator_type).replace("-", "_").lower()
        if self.generator_type not in {"encoder_decoder", "decoder_only"}:
            raise ValueError("generator_type must be 'encoder_decoder' or 'decoder_only'")

        trg_dim = self.embed_layer_dec.embedding_dim

        # 理论上 src_dim 和trg_dim == d_model，但为了代码健壮性，保留投影层
        # 如果维度一致（现在的设计目标），这就只是一个 Identity，不消耗显存
        self.src_project = nn.Linear(src_dim, d_model) if src_dim != d_model else nn.Identity()
        self.trg_project = nn.Linear(trg_dim, d_model) if trg_dim != d_model else nn.Identity()

        self.pos_encoder = PositionalEncoding(d_model, max_len=max_len)

        if self.generator_type == "encoder_decoder":
            self.transformer = nn.Transformer(d_model=d_model,
                                              nhead=nhead,
                                              num_encoder_layers=num_encoder_layers,
                                              num_decoder_layers=num_decoder_layers,
                                              dim_feedforward=dim_feedforward,
                                              dropout=dropout)
        else:
            decoder_layer = nn.TransformerDecoderLayer(
                d_model=d_model,
                nhead=nhead,
                dim_feedforward=dim_feedforward,
                dropout=dropout,
            )
            self.decoder = nn.TransformerDecoder(
                decoder_layer,
                num_layers=num_decoder_layers,
            )

        # 输出投影
        self.out_project = nn.Linear(d_model, trg_dim) if d_model != trg_dim else nn.Identity()

    def forward(self, trg_ids: torch.Tensor, vq_weight: torch.Tensor) -> torch.Tensor:
        # vq_weight 的形状应当是 [K_src, D]
        # 直接对其进行投影和位置编码，增加 seq_len 维度使其变为 [K_src, 1, D] (假设 batch_first=False)
        src_emb = self.src_project(vq_weight).unsqueeze(1)
        src_emb = self.pos_encoder(src_emb)

        # [K_trg, 1] -> [K_trg, 1, D]
        trg_emb = self.embed_layer_dec(trg_ids)
        trg_emb = self.trg_project(trg_emb)
        trg_emb = self.pos_encoder(trg_emb)

        # Transformer Forward
        # encoder_decoder: source codebook is contextualized by encoder first.
        # decoder_only: source codebook is used directly as decoder memory/K,V.
        if self.generator_type == "encoder_decoder":
            output = self.transformer(src=src_emb, tgt=trg_emb)
        else:
            output = self.decoder(tgt=trg_emb, memory=src_emb)

        # [K_trg, 1, trg_dim]
        output = self.out_project(output)

        return output.squeeze(1) # [K_trg, trg_dim]
