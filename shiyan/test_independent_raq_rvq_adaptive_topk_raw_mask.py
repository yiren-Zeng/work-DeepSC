"""Evaluate independent two-stage RAQ-RVQ with explicit Top-K masks."""
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "experiments"
    / "eval"
    / "independent_raq_rvq_src64_64_d2_adaptive_topk_raw_mask_combined"
)
...
def _summary_rows(payload: Dict[str, object]) -> List[Dict[str, object]]:
    ...
    for point in payload["results"]:
        ...
        base = {
            ...
            "raw_mask_bits": rate["raw_mask_bits"],
            "raw_source_bits": rate["raw_source_bits"],
            "raw_source_bpp": rate["raw_source_bpp"],
            "raw_coded_bits": rate["raw_coded_bits"],
            "raw_coded_bpp": rate["raw_coded_bpp"],
            "raw_channel_symbols": rate["raw_channel_symbols"],
            ...
            rows.append(
                {
                    **base,
                    ...
                    "raw_mask_bit_errors": channel["segment_bit_errors"][
                        "mask_raw"
                    ],
...
def run(args):
    ...
    for scan_id, target_rates in enumerate(activation_points):
        ...
        packets, selection_records = prepare_topk_raw_mask_packets(
            model,
            samples,
            target_rates,
            rvq_k_lists,
        ...
        rate = summarize_raw_mask_packets(
        ...
        print(
            "  explicit mask bits/image="
            f"{[scale['raw_mask_source_bits_mean_per_image'] for scale in rate['per_scale']]}"
        )
        print(
            f"  combined source={rate['raw_source_bits']/len(packets):.2f} bit/image, "
            f"coded={rate['raw_coded_bits']/len(packets):.2f} bit/image, "
            f"symbols={rate['raw_channel_symbols']/len(packets):.2f}/image, "
            f"no-channel PSNR={nochannel['psnr']:.6f}"
        )

        channel_results = []
        for snr in args.snrs:
            channel = evaluate_raw_mask_packets_over_channel(
                model,
                packets,
                float(snr),
                ldpc_code,
            ...
            print(
                ...
                f"mask errors={channel['segment_bit_errors']['mask_raw']}"
            )
        results.append(
            {
                "scan_id": scan_id,
    ...
    output = {
        "schema_version": 1,
        "evaluation": (
            "independent_raq_rvq_per_image_per_scale_topk_explicit_"
            "raw_mask_combined_ldpc"
        ),
        ...
        "transport": {
            ...
            "logical_segment_order": (
                "all first-stage scales, all explicit one-bit masks, then "
                "all compact active second-stage scales"
            ),
            "mask_scan_order": "row-major raster order",
            "mask_coding": "one explicit bit per token",
            "mask_bits_per_image_per_scale": [1024, 256],
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Independent RAQ-RVQ Top-K explicit-mask baseline with one "
            "combined LDPC stream per image."
        )
    )
