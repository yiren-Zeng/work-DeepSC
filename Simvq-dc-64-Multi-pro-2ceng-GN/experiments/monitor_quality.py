...
METRICS_PATH = ROOT / "experiments" / f"{EXPERIMENT_NAME}_epoch_metrics.csv"
CHECKPOINT_PATH = ROOT / "experiments" / "checkpoints" / EXPERIMENT_NAME / "best_vq_deepsc.pth"
SNAPSHOT_DIR = ROOT / "experiments" / "snapshots" / EXPERIMENT_NAME
LOG_DIR = ROOT / "experiments" / "logs"
SCREENING_PATH = ROOT / "experiments" / f"{EXPERIMENT_NAME}_screening.csv"
TARGET_PSNR = 32.30
TARGET_MS_SSIM = 0.9777
MAX_EPOCH = int(os.environ.get("SIMVQ_NUM_EPOCHS", "200"))
POLL_SECONDS = 60
...
def completed_epochs():
    ...
    return [
        ...
        if row["run_id"].startswith(EXPERIMENT_NAME)
    ]
...
def evaluate(epoch):
    ...
    log_path = LOG_DIR / f"{EXPERIMENT_NAME}_screen_epoch_{epoch:03d}.log"
    env = dict(os.environ)
...
def full_chain_confirmation(epoch):
    ...
    log_path = LOG_DIR / f"{EXPERIMENT_NAME}_real_chain_epoch_{epoch:03d}.log"
    env = dict(os.environ)
    env["CUDA_VISIBLE_DEVICES"] = env.get("SCREEN_GPU_ID", "3")
    command = [
        sys.executable, "-u", "test_real.py", "--checkpoint", str(snapshot),
