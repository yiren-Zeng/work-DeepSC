...
def main():
    ...
    stop_after_epoch = int(os.environ.get("SIMVQ_STOP_AFTER_EPOCH", "0"))
    ...
    if stop_after_epoch:
        print(f"[Info] This invocation will pause after epoch {stop_after_epoch} for screening.")
    ...
    # Only applies if a later variant introduces BatchNorm layers.
    if accumulation_steps > 1:
    ...
    for epoch in range(start_epoch, cfg.NUM_EPOCHS):
        ...
        if stop_after_epoch and epoch + 1 >= stop_after_epoch:
            print(f"[Info] Screening pause reached after epoch {epoch + 1}.")
            break

    writer.close()
    print("Training invocation complete.")
