...
#         return 0.5 * torch.erfc(x / math.sqrt(2))
#
#     def compute_ber(self, snr_db, n_block=None, rc=None):
#         """
#         基于有限码长理论计算 BER。
#         Args:
#             snr_db: 信噪比
#             n_block: 编码后的块长度 (bits), 即 L (e.g. 256, 512).
#                      如果传入此参数，将覆盖初始化时的 self.L_channel_uses
#             rc: 当前的信道编码率
#         """
#         real_rc = rc if rc is not None else self.R_c
#
#         # === 1. 确定物理信道使用次数 (Channel Uses) L_uses ===
#         if n_block is not None:
#             # 如果传入了 n (bits), L_uses = n / M
#             # 注意：这里 n_block 可能是 tensor，无需 device转换
#             L_uses_tensor = n_block / self.M_bits
#         else:
#             # 否则使用初始化时的默认值
#             L_uses_tensor = torch.tensor(self.L_channel_uses, device=self.device).float()
#
#         # === 传输速率 R ===
#         # R = 编码率(bits/coded_bit) * 调制阶数(coded_bits/symbol)
#         R_transport = real_rc * self.M_bits
...
#
#         # FBL 误差概率公式 (论文 Eq. 8): Q( sqrt(L) * (C - R) / sqrt(V) )
#         q_arg = torch.sqrt(L_uses_tensor) * (C - R_transport) / torch.sqrt(V)
#
#         # 包错误率 (Block Error Rate)
#         rho = self.q_function(q_arg)
#
#         # 近似 BER
#         rho = torch.clamp(rho, min=1e-12, max=1.0)
#         k_info_bits = R_transport * L_uses_tensor
#         ber = rho / k_info_bits
#
#         # 物理限制：BER 最大 0.5
#         ber = torch.clamp(ber, max=0.5)
