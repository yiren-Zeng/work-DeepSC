...
from communications.modulation import qpsk_modulate, qpsk_demodulate, qpsk_llr, bpsk_modulate, bpsk_demodulate, bpsk_llr
...
def evaluate_metrics_with_channel(model, loader, k_trg, target_snr, ldpc_code, device):
    ...
    for real_image in loader:
        ...
        # 4. BPSK 调制
        symbols = qpsk_modulate(coded_bits_tensor)

        # 5. AWGN 信道加噪
        noisy_symbols = awgn_channel(symbols, target_snr)

        # 6. BPSK 软解调与 LDPC 译码
        llrs = qpsk_llr(noisy_symbols, target_snr, device)
        ...
        # 8. 信源解码与重建
        reconstructed_images_raq = model.reconstruct_from_indices(recovered_indices_list, out["codebooks"])
...
@torch.no_grad()
def evaluate_metrics_with_channel_withoutLDPC(model, loader, k_trg, target_snr, ldpc_code, device):
    """包含完整物理层链路的 MS-SSIM 和 PSNR 联合评估"""
