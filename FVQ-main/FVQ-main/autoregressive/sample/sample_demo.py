...
from autoregressive.train.vq_model_qbridge import VQ_models
...
def main(args):
    ...
    folder_name = f"{model_string_name}-{ckpt_string_name}-size-{args.image_size}-size-{args.image_size_eval}-{args.vq_model}-" \
                  ...
                  f"cfg-{args.cfg_scale}-seed-{args.global_seed}/label-{args.cls_label}"
    ...
    for _ in pbar:
        ...
        c_indices = torch.full_like(c_indices, args.cls_label).to(device=device, non_blocking=True)
        qzshape = [len(c_indices), args.codebook_embed_dim, latent_size, latent_size] # [B, D, H, W]
        ...
        # index_sample: [B, L]
        samples = vq_model.decode_code(index_sample, qzshape, label=torch.full_like(c_indices, 1000).to(device=device, non_blocking=True)) # output value is between [-1, 1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cls-label", type=int, default=10)
