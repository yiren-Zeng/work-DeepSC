...
def main(args):
    ...
    # training args
    logger.info(f"==============================================")
    logger.info(f"======= THIS IS VFQ TRAINING =======")
    logger.info(f"==============================================")
    ...
    # Setup optimizer
    tlr = args.lr * (args.global_batch_size // 256)
    optimizer = creat_optimizer(model, args.weight_decay, tlr, (args.beta1, args.beta2), logger)
    ...
    def combined_lr_lambda(current_step: int):
        warmup_steps = 0 * len(loader)  # 每个 epoch 的 step 数
    ...
    aug_info = 2 * aug_info if dataset.aug_feature_dir is not None else aug_info # 1.1 and 1.05 size
    ...
    if args.gpt_ckpt:
        ...
        start_epoch = checkpoint["epoch"] if "epoch" in checkpoint else int(all_ckpt[0].split('/')[-1].split('.')[0])
        train_steps = checkpoint["steps"] if "steps" in checkpoint else int(start_epoch * int(len(dataset) / args.global_batch_size))
        del checkpoint
        ...
        if len(all_ckpt) == 0:
            ...
            del checkpoint
            logger.info(f"Resume training from checkpoint: {all_ckpt[0]}")
            logger.info(f"Initial state: steps={train_steps}, epochs={start_epoch}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            x = x.to(device, non_blocking=True) # (b, 1, 576)
            y = y.to(device, non_blocking=True)
            z_indices = x.reshape(x.shape[0], -1) # (b, 576)
            ...
            if train_steps % args.log_every == 0:
                ...
                logger.info(f"(step={train_steps:07d}) Train Loss: {avg_loss:.4f}, lr: {optimizer.param_groups[0]['lr']}, Train Steps/Sec: {steps_per_sec:.2f}")
        ...
        # Save checkpoint:
        if (epoch+1) % args.ckpt_every == 0 and train_steps > 0:
            if rank == 0:
                if not args.no_compile:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt-every", type=int, default=5) # save ckpt every 5 eps
