...
def eval_fid(args, vq_model, data_val, dir_raw, dir_recon, feature_extractor_path):
    vq_model.eval()
    dataloader = prepare_eval_data(data_val, args)
    total_count = 0  # Initialize total count
    for files, imgs, labels in tqdm(dataloader, dynamic_ncols=False, leave=True, file=sys.stderr):
    ...
    if args.rank == 0:
        ...
        jpeg_files = [f for f in os.listdir(dir_recon) if f.lower().endswith('.jpg') and os.path.isfile(os.path.join(dir_recon, f))]
        assert len(jpeg_files) == 5000, f"Expected 5000 JPG files, got {len(jpeg_files)}"
...
class PlainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return img_file, img, torch.tensor(0)
...
if __name__ == '__main__':
    ...
    sample_folder_dir = sample_folder_dir + "_fullvq"
    if args.rank == 0:
    ...
    eval_fid(args, vq_model, data_val, dir_raw, dir_recon, feature_extractor_path)
