...
class VectorQuantizer2(nn.Module):
    ...
    def __init__(
        ...
        default_qresi_counts=0, v_patch_nums=None, quant_resi=0.5, share_quant_resi=4,  QB_type='QBridge-L/4', show_usage=True, # share_quant_resi: args.qsr
    ):
        super().__init__()
        ...
        self.h_, self.w_ = int(self.vocab_size ** 0.5), int(self.vocab_size ** 0.5)
        
        self.qbridge = QBridge_models[QB_type](input_size=self.h_, is_uncondition=True, in_channels=self.Cvae)
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            emb_weights = self.embedding.weight.permute(1, 0).reshape(1, self.Cvae, self.h_, self.w_)
            emb_weights = self.qbridge(emb_weights)
            emb_weights = emb_weights.reshape(1, self.Cvae, self.vocab_size).permute(0, 2, 1).squeeze()
            
            embedding = emb_weights
            
            mean_vq_loss: torch.Tensor = 0.0
            mean_cmt_loss: torch.Tensor = 0.0
            ...
            for si, pn in enumerate(self.v_patch_nums): # from small to large
                ...
                if self.using_znorm:
                    ...
                    rest_NC = F.interpolate(f_rest, size=(pn, pn), mode='area').permute(0, 2, 3, 1).reshape(-1, C) if (si != SN-1) else f_rest.permute(0, 2, 3, 1).reshape(-1, C)
                    d_no_grad = torch.sum(rest_NC.square(), dim=1, keepdim=True) + torch.sum(embedding.data.square(), dim=1, keepdim=False)
                    d_no_grad.addmm_(rest_NC, embedding.data.T, alpha=-2, beta=1)  # (B*h*w, vocab_size)
                    idx_N = torch.argmin(d_no_grad, dim=1)
                
                z_q = embedding[idx_N].view(B, pn, pn, self.Cvae)
                # calc loss
                idx_Bhw = idx_N.view(B, pn, pn)
                h_BChw = F.interpolate(z_q.permute(0, 3, 1, 2), size=(H, W), mode='bicubic').contiguous() if (si != SN-1) else z_q.permute(0, 3, 1, 2).contiguous()
                h_BChw = self.quant_resi[si/(SN-1)](h_BChw)
                f_hat = f_hat + h_BChw
                f_rest -= h_BChw
                
                if self.show_usage:
                    cur_len = idx_N.shape.numel()
                    self.codebook_used[:-cur_len] = self.codebook_used[cur_len:].clone()
                    self.codebook_used[-cur_len:] = idx_N.view(-1)
        
                mean_vq_loss += F.mse_loss(f_hat, f_no_grad)
                mean_cmt_loss += F.mse_loss(f_hat.data, f_BChw).mul_(self.beta)
            mean_vq_loss *= 1. / SN
            mean_cmt_loss *= 1. / SN
            f_hat = (f_hat.data - f_no_grad).add_(f_BChw)
        usages = 0.0
        if self.show_usage:  
            usages = len(torch.unique(self.codebook_used)) / self.vocab_size
        return f_hat, (mean_vq_loss, mean_cmt_loss, torch.zeros_like(mean_vq_loss), usages), (None, None, None)#, mean_vq_loss_list
    # ===================== `forward` is only used in VAE training =====================
