...
class VQVAE(nn.Module):
    def __init__(
        ...
        self.quantize: VectorQuantizer2 = VectorQuantizer2(
            ...
            default_qresi_counts=default_qresi_counts, v_patch_nums=v_patch_nums, quant_resi=quant_resi, share_quant_resi=share_quant_resi, QB_type=QB_type,
        )
    ...
    def forward(self, inp, ret_usages=False):   # -> rec_B3HW, idx_N, loss
        ...
        f_hat, emb_loss, _ = self.quantize(self.quant_conv(self.encoder(inp)), ret_usages=ret_usages)
        return self.decoder(self.post_quant_conv(f_hat)), emb_loss, #vq_loss_list
    # ===================== `forward` is only used in VAE training =====================
