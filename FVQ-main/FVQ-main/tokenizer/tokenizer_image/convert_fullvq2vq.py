...
from vq_model_qbridge import VQ_models as FullVQ

from torch.utils.data import Dataset, DataLoader
from qbridge import QBridge_models

from vq_model import VQ_models as VQGAN


def normalize_01_into_pm1(x):  # normalize x from [0, 1] to [-1, 1] by (x*2) - 1
    return x.add(x).add_(-1)

def eval_fid(args, vqgan, data_val, dir_raw, dir_recon, feature_extractor_path):
    vqgan.eval()
    ...
    for files, labels, imgs in tqdm(dataloader, dynamic_ncols=False, leave=True, file=sys.stderr):
        ...
        with torch.no_grad():
            latent, _, [_, _, indices] = vqgan.encode(imgs)
            rec_imgs = vqgan.decode_code(indices, latent.shape)
    ...
    if args.rank == 0:
        ...
        result_file = os.path.join(args.sample_dir, "result.txt")
        with open(result_file, 'w') as f:
            print("rfid: %f, IS: %f " % (rfid, isc), file=f)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--fullvq-model", type=str, choices=list(FullVQ.keys()), default="VQ-16")
    parser.add_argument("--vqgan-model", type=str, choices=list(VQGAN.keys()), default="VQ-16")
    ...
    del checkpoint
    # 删除quantize.codebook_used
    
    
    vqgan = VQGAN[args.vq_model](
        codebook_size=args.codebook_size,
        codebook_embed_dim=args.codebook_embed_dim,
        codebook_l2_norm=args.codebook_l2_norm,
        )
    ...
    # 输出fullvq codebook_target
    codebook_target = vq_model.get_codebook(torch.full((1,), 1000).to('cuda'))
    model_weight['quantize.embedding.weight'] = codebook_target.squeeze()
    

    # 去除 model_weight 中 quantize.qbridge 的参数
    keys_to_remove = [key for key in model_weight.keys() if key.startswith("quantize.qbridge")]
    
    for key in keys_to_remove:
        del model_weight[key]
        
    model_weight['quantize.codebook_used'] = vqgan.state_dict()['quantize.codebook_used']

    vqgan.load_state_dict(model_weight, strict=True)
    
    del vq_model
    
    checkpoint = {"model": vqgan.state_dict()}
    
    
    if args.rank == 0:
        fullvq2vqgan_dir = ''
        os.makedirs(fullvq2vqgan_dir, exist_ok=True)
        torch.save(checkpoint, f"{fullvq2vqgan_dir}/fvq2vqgan_16384_40ep.pt")
    ...
    sample_folder_dir = sample_folder_dir + "_qbridge-vqgan"
    if args.rank == 0:
    ...
    feature_extractor_path = './data/evaluator_gen/weights-inception-2015-12-05-6726825d.pth'
    eval_fid(args, vqgan, data_val, dir_raw, dir_recon, feature_extractor_path)
