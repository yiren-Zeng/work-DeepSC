...
def eval_fid(args, vq_model, dir_val, dir_raw, dir_recon, feature_extractor_path):
    vq_model.eval()
    dataloader = prepare_eval_data(dir_val, args)
    total_count = 0  # Initialize total count
    for files, imgs in tqdm(dataloader):
        imgs = imgs.to('cuda', non_blocking=True)
        with torch.no_grad():
            latent, _, [_, _, indices] = vq_model.encode(imgs)
            rec_imgs = vq_model.decode_code(indices, latent.shape)
...
if __name__ == '__main__':
    ...
    eval_fid(args, vq_model, dir_val, dir_raw, dir_recon, feature_extractor_path)
