...
from tokenizer.tokenizer_image.vq_model_qbridge_release import VQ_models
...
def main(args):
    ...
    def combined_lr_lambda(current_step: int):
        ...
        if current_step < warmup_steps:
            ...
            elif args.sche_type == 'cos':
                pasd = (current_step - warmup_steps) / (total_steps-1 - warmup_steps)
                if pasd < T: 
                    return 1.0
                else:
                    # 余弦退火：从1.0衰减到wpe
                    cos_progress = (pasd - T) / max_rest  # 在[0,1]范围内的进度
                    return wpe + (1 - wpe) * 0.5 * (1 + math.cos(math.pi * cos_progress))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sche_type", type=str, choices=["lin", "const", "cos"], default="lin")
