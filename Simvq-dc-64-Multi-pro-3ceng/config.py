...
class Config:
    IN_CHANNELS = 3
    OUT_CHANNELS = 3
    NUM_DOWNSAMPLE_BLOCKS = 3
    BASE_CHANNELS = 64

    # 3组参数（第一层4倍下采样，后两层2倍下采样）
    EMBEDDING_DIM_LIST = [128, 256, 512]
    NUM_EMBEDDINGS_LIST = [2, 4, 256]
    COMMITMENT_COST = 0.25

    # 各层下采样步幅 [Layer0=4倍, Layer1=2倍, Layer2=2倍]
    DOWNSAMPLE_STRIDES = [4, 2, 2]

    # 各层VQ损失权重 [Layer0, Layer1, Layer2]
    # 初始值（阶段1），会按调度计划退火
    LAYER_LOSS_WEIGHTS_INIT = [1, 5, 10]
    # 最终值（阶段3）
    LAYER_LOSS_WEIGHTS_FINAL = [1, 1, 1]

    # 各层跳跃连接 Dropout 概率 [Layer0, Layer1]
    # 初始值（阶段1），会按调度计划衰减
    SKIP_DROPOUT_P_INIT = [0.5, 0.05]
    # 最终值（阶段2结束/阶段3）
    SKIP_DROPOUT_P_FINAL = [0.0, 0.0]
