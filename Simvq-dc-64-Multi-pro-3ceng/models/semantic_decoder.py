...
class SemanticDecoder(nn.Module):
    """
    三层解码器: 3次上采样 + skip connection
    前两层2倍上采样，最后一层4倍上采样（与编码器 [4x,2x,2x] 下采样对称）
    输入 [F̂1,F̂2,F̂3] → 输出 (B,3,256,256)
    """
    def __init__(self, embedding_dims, out_channels, up_mode: str = "nearest",
                 skip_dropout_p=None, upsample_scales=None):
        """
        embedding_dims: 各尺度量化特征通道列表（如 [128,256,512]，与编码器输出对应）
        out_channels:   输出图像通道数（RGB=3）
        up_mode:        上采样方式（"nearest" 或 "bilinear"）
        skip_dropout_p: 各层跳跃连接 Dropout 概率列表 [Layer0, Layer1]（None 或空列表表示不使用）
        upsample_scales: 各层上采样倍率列表（如 [2, 2, 4]），默认全为2
        """
        super().__init__()
        ...
        if skip_dropout_p and len(skip_dropout_p) >= num_skip:
            # skip_dropout_p = [Layer0_p, Layer1_p]
            # 循环中 i=0 → Layer1, i=1 → Layer0，所以反序
        ...
        for i in range(self.L):  # i=0..L-1；总计 L 个
            if i < self.L - 1:
                ...
                out_ch = self.embedding_dims[-2 - i]  # 依次为 256, 128 ...
            else:
    ...
    def set_skip_dropout_p(self, p_list):
        ...
        p_list: [Layer0_p, Layer1_p]（按层号排列）
        """
    ...
    def forward(self, quant_feats):
        ...
        quant_feats: list[Tensor]，从浅到深排列：
            [F̂1(B,128,64,64), F̂2(B,256,32,32), F̂3(B,512,16,16)]
        ...
        for i, block in enumerate(self.up_blocks):
            ...
            if i < self.L - 1:
                skip = quant_feats[-2 - i]  # 依次取 F̂2, F̂1
