...
def test_real():
    ...
    print(f"调制: QPSK")
    print(f"RAQ 目标码本大小 (Target K): {cfg.RAQ_N_TRG}")
    ...
    deepsc_model = DeepSC(
        ...
        raq_n_trg=cfg.RAQ_N_TRG,
        device=device
    ).to(device)

    checkpoint_path = os.path.join("/workspace/yi/work/Fixed-Raq-dc-64/checkpoints/best_vq_deepsc.pth")
    ...
    for snr in TEST_SNRS:
        ...
        mean_ms_ssim, mean_psnr = evaluate_metrics_with_channel(
            deepsc_model, test_dataloader, snr, ldpc_code, device)
    ...
    print("\n" + "=" * 40)
    print("=== RAQ 最终测试结果 ===")
    print(f"Target K: {cfg.RAQ_N_TRG}")
    print("=" * 40)
...
def evaluate_metrics_with_channel(model, loader, target_snr, ldpc_code, device):
    ...
    for real_image in loader:
        ...
        # 1. 信源编码与量化
        out = model.forward_test_raq(real_image)

        # 2. 索引转比特流
        k_trg_list = [model.raq_n_trg] * len(out["indices_raq"])
        flat_bits_raq, original_spatial_dims_raq, original_num_embeddings_raq = indices_to_bits(
            out["indices_raq"], k_trg_list)
