...
class RAQ(nn.Module):
    def __init__(self, embedding_dim: int, n_embed_src: int, n_embed_trg: int,
                 device: str = "cuda:2"):
        super().__init__()
        self.device = torch.device(device)
        self.embedding_dim = embedding_dim
        self.n_embed_src = n_embed_src
        self.n_embed_trg = n_embed_trg

        # === SimVQ 改进: 使用 ProjectedEmbedding (冻结底层 + 可训练投影层) ===
        self.trg_embed = ProjectedEmbedding(n_embed_trg, embedding_dim).to(self.device)
    ...
    def generate_codebook_transformer(self, vq_weight: torch.Tensor) -> torch.Tensor:
        trg_ids = torch.arange(self.n_embed_trg, device=self.device, dtype=torch.long).unsqueeze(1)
