...
class DeepSC(nn.Module):
    ...
    def sample_mod_bits(self, snr_db):
        if snr_db < 4.0:
    ...
    def forward_train_raq(self, x):
        ...
        current_mod_bits = self.sample_mod_bits(snr_db)
        ...
        for i, feat in enumerate(encoder_features):
            K_tilde = self.raq_n_trg

            current_vq_weight = self.vector_quantizers[i].codebook.projected_weight()
            W_trg = self.raqs[i].generate_codebook_transformer(current_vq_weight)
    ...
    def forward_val_raq(self, x):
        ...
        for i, feat in enumerate(encoder_features):
            ...
            vq_loss_t, _, encoding_idx_t = self.vector_quantizers[i].forward_raq(feat, W_trg)
            vq_losses_raq.append(vq_loss_t)
            indices_raq.append(encoding_idx_t)
    ...
    def compute_codebook_utilization(self, dataloader, max_batches=None, device=None):
        ...
        for i in range(num_layers):
            ...
            raq_stats = VectorQuantizer.compute_codebook_stats(raq_all, self.raq_n_trg)
    ...
    def print_codebook_utilization(results, num_embeddings_list=None):
        ...
        for i in range(num_layers):
            ...
            # RAQ 支路
            r = results['raq'][i]
            k_raq = num_embeddings_list[i] if num_embeddings_list else "?"
