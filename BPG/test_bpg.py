...
def main():
    ...
    # ================= 参数配置 =================
    # 原始未压缩图片所在的文件夹路径
    ori_path = '/workspace/yi/work/BPG/kodak'

    # 已经解压完毕的图片文件夹路径
    rx_path = '/workspace/yi/work/BPG/kodak-decompress-0.5-bpsk-ratio-0.28'
    ...
    for k in tqdm(range(1, 25), desc="Evaluating"):
        ...
        rx_png_file = os.path.join(rx_path, img_name)

        # 【补全3】判断解压图片是否存在（对应后面的 else 悬崖效应）
        if os.path.exists(rx_png_file):
            # 读取原图和恢复图，并强制转换为 RGB 色彩模式
            img_ori_pil = Image.open(ori_file).convert('RGB')
            img_dec_pil = Image.open(rx_png_file).convert('RGB')
