def compute_schedule(epoch, num_epochs, cfg):
    ...
    # Official EMA-RQ returns an unweighted commitment loss for every U-Net
    # scale.  Its global 0.25 coefficient is applied by DeepSCLoss, so the
    # per-scale schedule must remain fixed at one throughout training.
    if str(getattr(cfg, "QUANTIZER_TYPE", "")).lower() == "rq_ema":
        loss_weights = [1.0] * int(cfg.UNET_DEPTH)
