...
class DeepSCLoss(nn.Module):
    """
    纯 SRC 单支路损失 + 多尺度量化损失 + 可选的 VGG 感知损失。

    ``quantization_weight`` is deliberately applied here, once, so an
    EMA-RQ quantizer can return the raw cumulative commitment loss used by the
    official RQ-VAE implementation.  Legacy quantizers keep the default 1.0
    because their returned losses already contain their own coefficients.
    """
    def __init__(self, layer_weights=None, mse_weight=1.0, ms_ssim_weight=0.0,
                 lpips_weight=0.0, quantization_weight=1.0):
        super().__init__()
    ...
    def forward(self, x, x_hat, vq_losses):
        if len(vq_losses) != len(self.layer_weights):
            raise ValueError(
                f"Expected {len(self.layer_weights)} quantization losses, got {len(vq_losses)}"
            )
        ...
        # 按层权重加权VQ损失
        weighted_vq = self.quantization_weight * sum(
