#!/bin/bash
# Fresh Stage-B two-scale Residual-SimVQ training at 4608 source bits/image.
set -euo pipefail
...
# Preserve the established 256px Resize+RandomCrop/CenterCrop preprocessing.
...
# This family always starts from scratch and never inherits another quantizer.
...
EXPERIMENT_NAME="quality_v2_B_larger_rate047_residual_simvq_unet2_ds8x2_k4-2_d2-2"
RUN_ID="residual_simvq_k4-2_d2-2_rate047-$(date +%Y%m%d-%H%M%S)"
export EXPERIMENT_RUN_ID="$RUN_ID"

echo "Experiment: $EXPERIMENT_NAME"
echo "Run ID: $RUN_ID"
echo "GPU: $GPU_ID"
echo "Quantizer: residual_simvq, shared projected codebook, K=[4,2], depth=[2,2]"
echo "Optimizer: codebook.proj lr=2e-4; frozen codebook.embed excluded"
