#!/bin/bash
# Rate-0.041 Variant B: layer-1 channel-wise SimVQ/CVQ, layer-2 patch-wise SimVQ.
set -euo pipefail
...
RUN_ID="exp13_rate041_B_hybridcvq_cb65536-8192-$(date +%Y%m%d-%H%M%S)"
...
if [ "${DEBUG_PDB:-0}" = "1" ]; then
  ...
  DEBUGPY_PORT="${DEBUGPY_PORT:-5680}"
