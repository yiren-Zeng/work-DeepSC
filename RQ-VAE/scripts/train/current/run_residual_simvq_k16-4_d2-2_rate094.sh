#!/bin/bash
# Fresh Stage-B two-scale Residual-SimVQ training at 9216 source bits/image.
set -euo pipefail
...
# This variant starts from scratch and never inherits another quantizer.
...
EXPERIMENT_NAME="quality_v2_B_larger_rate094_residual_simvq_unet2_ds8x2_k16-4_d2-2"
RUN_ID="residual_simvq_k16-4_d2-2_rate094-$(date +%Y%m%d-%H%M%S)"
export EXPERIMENT_RUN_ID="$RUN_ID"

echo "Experiment: $EXPERIMENT_NAME"
echo "Run ID: $RUN_ID"
echo "GPU: $GPU_ID"
echo "Quantizer: residual_simvq, shared projected codebook, K=[16,4], depth=[2,2]"
