#!/bin/bash
# Adaptive rq_ema explicit-mask evaluation over Sionna LDPC 1/2+BPSK+AWGN.
set -euo pipefail
...
OUTPUT_DIR="experiments/adaptive_eval/quality_v2_B_larger_rate047_rq_ema_unet2_ds8x2_k4-2_d2-2/ldpc_bpsk_explicit_mask"
mkdir -p "$OUTPUT_DIR" experiments/logs
RUN_ID="rq_ema_adaptive_explicit_mask_ldpc_bpsk_gpu${GPU_ID}-$(date +%Y%m%d-%H%M%S)"
...
python -u test_adaptive_ldpc.py \
