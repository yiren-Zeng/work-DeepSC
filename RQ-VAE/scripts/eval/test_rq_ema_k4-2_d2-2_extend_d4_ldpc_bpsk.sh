#!/bin/bash
# Full zero-training depth sweep plus fixed depth-4 LDPC 1/2 + BPSK test.
set -euo pipefail
...
# Keep the source architecture at [2,2]; the CLI extends it only after load.
...
OUTPUT_DIR="experiments/depth_extension_eval/quality_v2_B_larger_rate047_rq_ema_unet2_ds8x2_k4-2_d2-2/zero_training_shared_codebook_d1to4_depth4_ldpc_bpsk"
mkdir -p "$OUTPUT_DIR" experiments/logs
LOG_PATH="experiments/logs/eval_rq_ema_zero_train_depth4_ldpc_bpsk_gpu${GPU_ID}_$(date +%Y%m%d-%H%M%S).log"
