...
class VectorQuantizer(nn.Module):
    ...
    def _lookup_bhwc(self, inputs_bhwc, embed_weight=None):
        """Return raw projected-code lookup and spatial indices.

        This helper deliberately contains only the lookup portion of the
        original SimVQ forward pass.  Nearest-neighbour decisions remain
        detached inside :meth:`_nearest_code_indices`, while the selected
        projected embeddings retain gradients to ``codebook.proj``.
        """
        ...
        return quantized_bhwc, encoding_idx.view(B, H, W)
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        quantized_bhwc, encoding_idx = self._lookup_bhwc(
            inputs_bhwc, embed_weight=embed_weight
        )
