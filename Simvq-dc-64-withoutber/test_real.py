"""
Simvq-dc-64-withoutber: 测试真实重建质量（完整物理层链路）

此版本模型训练时没有添加信道噪声，但测试时通过完整物理层链路
（LDPC编码 → BPSK调制 → AWGN信道 → 软解调 → LDPC解码）
评估模型在不同 SNR 下的鲁棒性。
"""
...
def test_real():
    ...
    print("=" * 40)
    print(f"开始 SimVQ (withoutber) 支路真实环境测试 (Real Transmission Chain)")
    print(f"注意: 模型训练时无信道噪声，测试时加入完整物理层链路")
    ...
    checkpoint_path = os.path.join("/workspace/yi/work/Simvq-dc-64-withoutber/checkpoints/best_vq_deepsc.pth")
    ...
    for snr in TEST_SNRS:
        ...
        print(f"SNR {snr} dB | SimVQ (withoutber) Avg MS-SSIM: {mean_ms_ssim:.4f} | Avg PSNR: {mean_psnr:.4f} dB")

    print("\n" + "=" * 40)
    print("=== SimVQ (withoutber) 最终测试结果 ===")
...
def evaluate_metrics_with_channel(model, loader, num_embeddings_list, target_snr, ldpc_code, device):
    """包含完整物理层链路的 MS-SSIM 和 PSNR 联合评估 (SimVQ/withoutber版本)"""
