#!/bin/bash
# Stage B larger backbone with ViTvq NoCompress quantizers and codebooks [64,256].
set -euo pipefail
...
# Reuse the expanded backbone weights. SimVQ-only codebook parameters are
# intentionally skipped because this is a new independent quantizer experiment.
...
RUN_ID="exp7_larger_vitvq_nocompress_k64-256-$(date +%Y%m%d-%H%M%S)"
...
echo "Run ID: $RUN_ID"
echo "GPU: $GPU_ID"
echo "Quantizer: $SIMVQ_QUANTIZER_TYPE ($SIMVQ_VITVQ_QBRIDGE_TYPE)"
...
python -u tools/auto_evaluate_completed_experiments.py \
  --experiment "quality_v2_B_larger_ViTvqNoCompress_unet2_ds8x2_k64-256" \
  --gpu "$GPU_ID"
