#!/bin/bash
# Stage B larger backbone with original VQ quantizers and codebooks [128,16].
set -euo pipefail
...
# Reuse the same expanded backbone as the SimVQ [128,16] experiment. Quantizer
# tensors are intentionally not reused because this run trains original VQ
# embeddings instead of SimVQ projected codebooks.
...
RUN_ID="exp10_larger_cb128-16_vq-$(date +%Y%m%d-%H%M%S)"
...
echo "Run ID: $RUN_ID"
echo "GPU: $GPU_ID"
echo "Quantizer: $SIMVQ_QUANTIZER_TYPE (original trainable VQ embedding)"
...
python -u tools/auto_evaluate_completed_experiments.py \
  --experiment "quality_v2_B_larger_cb128-16_VQ_unet2_ds8x2_k128-16" \
  --gpu "$GPU_ID"
