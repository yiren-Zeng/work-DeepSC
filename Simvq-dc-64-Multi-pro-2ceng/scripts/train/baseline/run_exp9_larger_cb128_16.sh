#!/bin/bash
# Stage B larger SimVQ backbone with codebooks [128,16].
set -euo pipefail
...
RUN_ID="exp9_larger_cb128-16-$(date +%Y%m%d-%H%M%S)"
...
python -u tools/auto_evaluate_completed_experiments.py \
  --experiment "quality_v2_B_larger_cb128-16_unet2_ds8x2_k128-16" \
  --gpu "$GPU_ID"
